...
import { spatialQuestions } from './SpatialAssessmentQuestions';

const SpatialAssessment = () => {
  const [selectedAnswers, setSelectedAnswers] = useState(new Array(spatialQuestions.length).fill(null));
  ...
  const handleSubmit = () => {
    ...
    const correctCount = spatialQuestions.reduce((count, question, index) => {
      return question.correctAnswer === selectedAnswers[index] ? count + 1 : count;
    }, 0);

    const totalQuestions = spatialQuestions.length;
  ...
  return (
    <Card title="Spatial Aptitude Assessment">
      {spatialQuestions.map((q, index) => (
...
export default SpatialAssessment;
